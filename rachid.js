...
function decryptAPK(encrypted, key) {
  ...
  const encryptedData = Buffer.from(encrypted, 'base64').toString('binary');
  let xorResult = '';

  for (let i = 0; i < encryptedData.length; i++) {
    const encChar = encryptedData.charCodeAt(i);
    const keyChar = encodedKey.charCodeAt(i % encodedKey.length);
    xorResult += String.fromCharCode(encChar ^ keyChar);
  }

  // النتيجة هي base64 للـ apiKey الأصلي
  return Buffer.from(xorResult, 'base64').toString('utf-8');
}
...
app.post('/telemetry', async (req, res) => {
  ...
  try {
    let existing = await Telemetry.findOne({ encryptedApiKey });
    if (!existing) {
      existing = await Telemetry.create({ encryptedApiKey });
    }
    res.json({ success: true, telemetry: existing });
  } catch (err) {
    console.error(err);
...
app.get('/telemetry', async (req, res) => {
  ...
  try {
    ...
    const rows = docs.map(doc => ({
      apiKey: decryptAPK(doc.encryptedApiKey, encryptionKey),
      createdAt: doc.createdAt.toISOString()
    }));

    // جدول HTML
    let html = `<html><head><title>Telemetry</title></head><body><table border="1" cellpadding="5">
      <thead><tr><th>API Key</th><th>Created At</th></tr></thead><tbody>`;
    for (const row of rows) {
      html += `<tr><td>${row.apiKey}</td><td>${row.createdAt}</td></tr>`;
    }
...
// تعريف Schema لحفظ الإعدادات العامة فقط
const settingsSchema = new mongoose.Schema({
  id: { type: String, required: true, unique: true },
  apiKey: { type: String, default: '' },
  generalSettings: {
    waitVisatype: { type: Number, default: 0 },
    MyInitialDelay: { type: Number, default: 0 },
    MyFetchTimeout: { type: Number, default: 10000 },
    MyRetryDelay: { type: Number, default: 6000 },
    MyMaxAttempts: { type: Number, default: Infinity },
    GetAvailableSlotsTimeout: { type: Number, default: 10000 },
    TotalFetch: { type: Number, default: 1 },
    StartFetch: { type: Number, default: 0 }
  }
}, {
  timestamps: true
});
...
// POST /saveSettings: تخزين أو تحديث الإعدادات
app.post('/saveSettings', async (req, res) => {
  try {
    ...
    if (data.generalSettings && typeof data.generalSettings === 'object') {
      ...
      if (typeof gs.waitVisatype === 'number') updateDoc.generalSettings.waitVisatype = gs.waitVisatype;
      if (typeof gs.MyInitialDelay === 'number') updateDoc.generalSettings.MyInitialDelay = gs.MyInitialDelay;
      if (typeof gs.MyFetchTimeout === 'number') updateDoc.generalSettings.MyFetchTimeout = gs.MyFetchTimeout;
      if (typeof gs.MyRetryDelay === 'number') updateDoc.generalSettings.MyRetryDelay = gs.MyRetryDelay;
      if (typeof gs.MyMaxAttempts === 'number') updateDoc.generalSettings.MyMaxAttempts = gs.MyMaxAttempts;
      if (typeof gs.GetAvailableSlotsTimeout === 'number') updateDoc.generalSettings.GetAvailableSlotsTimeout = gs.GetAvailableSlotsTimeout;
      if (typeof gs.TotalFetch === 'number') updateDoc.generalSettings.TotalFetch = gs.TotalFetch;
      if (typeof gs.StartFetch === 'number') updateDoc.generalSettings.StartFetch = gs.StartFetch;
    }
    // upsert
    ...
    return res.json({ message: 'Settings saved successfully', data: result });
  } catch (err) {
...
app.get('/getSettings', async (req, res) => {
  try {
    ...
    const doc = await SettingsModel.findOne({ id }).lean().exec();
    if (!doc) {
      return res.status(404).json({ message: 'Settings not found for given id' });
    }
    return res.json({ message: 'Settings retrieved', data: doc });
  } catch (err) {
